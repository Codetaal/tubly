...
class Youtube
{
    ...
    public function __construct($app, Google_Client $client)
    {
        ...
        if ($accessToken = $this->getLatestAccessToken()) {
    ...
    {
        $params = array_filter(array('mine' => true, 'maxResults' => 25, 'onBehalfOfContentOwner' => '', 'onBehalfOfContentOwnerChannel' => ''));
        $response = $this->youtube->playlists->listPlaylists(
            'snippet, contentDetails',
            $params
        );

        $response = collect($response->items)->transform(function ($item, $key) {
            // add playlist ID to snippet data
            $item->snippet->playlist_id = $item->id;
            // misc
            $item->snippet->title = strtolower($item->snippet->title);

            return $item;

        })->pluck("snippet")->sortBy('title')->all();
    ...
    {
        return session()->put('access_token', json_encode($accessToken));
